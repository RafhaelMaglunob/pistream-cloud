...
def camera_thread():
    ...
    restart_delay = 2   # start small
    max_delay = 15      # cap backoff
    ...
    while True:
        ...
        try:
            ...
            while True:
                ...
                if not chunk:
                    # Read stderr to understand why it died
                    try:
                ...
                while True:
                    ...
                    # Sanity check: valid JPEG is at least 1KB
                    if len(frame) < 1024:
        ...
        if frames_captured == 0:
            ...
            consecutive_failures = 0
            restart_delay = 2  # reset on success
...
# ------------------------ DETECTION WORKERS ------------------------
def overlay_worker():
    """Runs at ~15fps — only does color detection + overlay rendering. Never blocked by ML."""
...
def ml_worker():
    ...
    while True:
        time.sleep(0.5)  # ~2fps max — Pi CPU can't handle more anyway
...
def generate_frames():
    ...
    while True:
        # Prefer overlay frame, fall back to raw frame so phone always gets something
        with overlay_lock:
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*45}")
    print(f"  Pi Camera Server running (HTTP)")
    print(f"  Stream:   http://{local_ip}:{selected_port}/stream")
    print(f"  Snapshot: http://{local_ip}:{selected_port}/snapshot.jpg")
    print(f"  Web UI:   http://{local_ip}:{selected_port}/")
    print(f"  Status:   http://{local_ip}:{selected_port}/status")
    print(f"{'='*45}\n")
