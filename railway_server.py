#!/usr/bin/env python3
"""
Pi Camera Relay Server — Fixed for Render/Railway
- Real-time SSE (~200ms latency instead of 1s)
- ML detection toggle relay to Pi
- Fixed ML results format for dashboard
- Faster frame push pipeline
"""
...
# ── ML state (cloud-side) ──────────────────────────────────────
ml_enabled_on_pi = False   # track whether we told Pi to enable ML
pi_ml_enabled    = False   # what Pi last reported
...
# ── SSE clients — use a list of queues ────────────────────────
sse_clients      = []
...
def send_crash_email(cam_label, side, conf, elapsed, snapshot_bytes=None):
    if not GMAIL_USER or not GMAIL_PASSWORD or not ALERT_EMAILS:
        print("[EMAIL] Not configured — set GMAIL_USER, GMAIL_PASSWORD, ALERT_EMAILS")
        return
...
# ═════════════════════════════════════════════════════════════
#  ML Toggle — relay to Pi via WebSocket command
# ═════════════════════════════════════════════════════════════
@app.route('/ml', methods=['GET', 'POST'])
@login_required
def ml_toggle():
    """Browser calls this to enable/disable ML on the Pi."""
    global ml_enabled_on_pi
    if request.method == 'POST':
        enabled = request.get_json(force=True).get('enabled', False)
        ml_enabled_on_pi = enabled
        # Relay the command to the Pi over WebSocket
        _send_to_pi({"type": "ml_toggle", "enabled": enabled})
        # Also try HTTP relay if WS is down (Pi polls /pi/ml_state)
        print(f"[ML] Browser set ML = {enabled}, relayed to Pi")
        return jsonify({"success": True, "enabled": enabled})
    return jsonify({"enabled": ml_enabled_on_pi})

@app.route('/pi/ml_state')
def pi_ml_state():
    """Pi polls this to know if ML should be on (fallback when WS unavailable)."""
    if not check_secret(): return 'Unauthorized', 401
    return jsonify({"enabled": ml_enabled_on_pi})
...
def stream_sse():
    """
    Real-time SSE stream. Each frame pushed by Pi arrives here within ~50ms.
    Use a small queue so we don't buffer stale frames.
    """
    q = queue.Queue(maxsize=3)  # small = low latency, not buffered
...
def get_ml():
    """
    Returns ML results. Normalizes keys so both '0'/'1' and 'front'/'rear'
    formats work for the dashboard JS.
    """
    with ml_lock:
        raw = ml_results.copy()

    # raw keys from Pi are "0" and "1"
    # Return BOTH formats so dashboard always finds what it needs
    normalized = {
        "0": raw.get("0", {}),
        "1": raw.get("1", {}),
        "front": raw.get("0", raw.get("front", {})),
        "rear":  raw.get("1", raw.get("rear",  {})),
        "ml_enabled": ml_enabled_on_pi,
    }
...
def test_email():
    if not GMAIL_USER:
        return "Not configured. Set GMAIL_USER, GMAIL_PASSWORD, ALERT_EMAILS.", 400
    send_crash_email("FRONT", "Center", 97.3, 3.2, None)
    return f"Test email sent to: {', '.join(ALERT_EMAILS) or 'nobody — set ALERT_EMAILS'}", 200


# ═════════════════════════════════════════════════════════════
#  Main Dashboard UI — fixed ML toggle + faster SSE display
# ═════════════════════════════════════════════════════════════
@app.route('/')
@login_required
def index():
...
#fps-counter{font-size:10px;color:var(--dim);position:absolute;
  bottom:6px;right:10px;background:rgba(0,0,0,.5);padding:2px 6px}
</style>
...
<div class="stream-box">
  ...
  <img   id="fallbackImg" alt="" style="display:block;width:100%;max-width:1280px;object-fit:contain">
...
<div class="grid">
  ...
  <div class="panel">
    ...
    <div class="sr"><span class="sk">ML STATE</span><span class="sv" id="s-ml">OFF</span></div>
    ...
    <div style="margin-top:10px">
      ...
      <button class="btn" onclick="doReconnect()">↻ Reconnect WebRTC</button>
      <button class="btn" style="color:var(--green);border-color:var(--green)"
        onclick="testEmail()">📧 Test Alert Email</button>
    </div>
  </div>
  ...
  <div class="panel" style="grid-column:span 2">
    ...
    <div id="ml-panel"><div class="ml-none">ML Detection: OFF — click "Enable ML Detection" to start</div></div>
  </div>
    ...
    <div style="font-size:11px;line-height:2">
      ...
      {% else %}
        ...
        <div style="color:var(--dim);margin-top:6px;font-size:10px">
          Add ALERT_EMAILS in environment vars
        </div>
      {% endif %}
    </div>
  </div>
...
function setStat(id,t,c){const e=document.getElementById(id);if(!e)return;e.textContent=t;e.className='sv'+(c?' '+c:'');}
function sleep(ms){return new Promise(r=>setTimeout(r,ms));}
setInterval(()=>{document.getElementById('clock').textContent=new Date().toTimeString().slice(0,8);},1000);

// ── SSE — real-time fallback ──────────────────────────────────
function startSSE(){
  if(sse) return;
  ...
  setStat('s-s','SSE ~200ms');
  setMsg('SSE stream active','var(--orange)');

  sse = new EventSource('/stream/sse');
  sse.onmessage = e => {
    // Update image src with new frame immediately
    document.getElementById('fallbackImg').src = 'data:image/jpeg;base64,' + e.data;
    document.getElementById('dot-pi').className = 'dot live';
    tickFPS();
    if(!rtcLive) setMsg('SSE live (~200ms latency)','var(--orange)');
  };
  sse.onerror = () => { stopSSE(); setTimeout(startSSE, 2000); };
}
function stopSSE(){
...
async function handleOffer(sdp){
  ...
  setMsg('Pi offer received — connecting WebRTC...','var(--yellow)');
  ...
  pc.ontrack=ev=>{
    ...
    document.getElementById('dot-pi').className='dot live';
    rtcLive=true;
    setMsg('⚡ WebRTC live — ultra low latency!','var(--green)');
  ...
  pc.onconnectionstatechange=()=>{
    ...
    if(s==='failed'||s==='disconnected'||s==='closed'){
      ...
      setMsg('WebRTC dropped — falling back to SSE...','var(--red)');
      startSSE();
...
// ── ML Toggle — sends command to cloud which relays to Pi ─────
function toggleML(){
  mlOn=!mlOn;
  ...
  if(mlOn){
    ...
    btn.textContent='⏹ Disable ML Detection';
    setStat('s-ml','ON ✓','ok');
    // POST to cloud server which relays to Pi
    fetch('/ml',{method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({enabled:true})})
      .then(r=>r.json())
      .then(d=>console.log('[ML] Enabled:', d))
      .catch(e=>console.error('[ML] Toggle failed:', e));
    mlTimer=setInterval(updateML, 800);
    updateML();
  } else {
    btn.className='btn';
    btn.textContent='▶ Enable ML Detection';
    setStat('s-ml','OFF');
    fetch('/ml',{method:'POST',
      headers:{'Content-Type':'application/json'},
    ...
    document.getElementById('ml-panel').innerHTML=
      '<div class="ml-none">ML Detection: OFF — click "Enable ML Detection" to start</div>';
...
function updateML(){
  fetch('/ml_results').then(r=>r.json()).then(data=>{
    // Support both key formats: "0"/"1" and "front"/"rear"
    const front = data['front'] || data['0'] || {};
    const rear  = data['rear']  || data['1'] || {};
    const cams  = [{d:front,k:'0',l:'FRONT',c:'ml-f'},{d:rear,k:'1',l:'REAR',c:'ml-r'}];

    let html='';
    cams.forEach(({d,k,l,c})=>{
...
function updateStatus(){
  fetch('/status').then(r=>r.json()).then(d=>{
    ...
    // Status keys can be "0"/"1" or "front"/"rear"
    s('s-f', d['0']||d['front']||'—');
    s('s-r', d['1']||d['rear'] ||'—');
  }).catch(()=>{});
}
...
if __name__ == '__main__':
    ...
    # Use gevent or threading for SSE to work properly on Render
